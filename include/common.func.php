...
function ShowMsg($msg, $gourl, $onlymsg=0, $limittime=0)
{
    ...
    $htmlhead  = "<html>\r\n<head>\r\n<title>织梦58提示信息</title>\r\n<meta http-equiv=\"Content-Type\" content=\"text/html; charset=gb2312\" />\r\n";
   $htmlhead .= "<base target='_self'/>\r\n<style>
	.ts_div{width:500px;overflow:hidden;margin:0 auto;margin-top:50px;border:1px solid #E9E9E9;border-radius:3px;}
...
.ts_div p{line-height:51px;background:url(/images/ts_bg.png) no-repeat 0 center;margin:10px auto;font-size:14px;text-align:left;text-indent:70px;}
.ts_tz{margin:10px auto;text-align:right;width:500px;color:#666;font-size:12px;margin-top:-35px;margin-left:260px;}
.ts_tz a{color:#f30; font-size:12px;}a.history{background:#fff url(/images/bga.png) no-repeat 0 -120px;text-indent:-9999px;float:left;margin:-27px 0 0 15px;margin-top:-29px\9;display:inline;width:13px;height:15px;overflow:hidden;}a:hover.history{background-position:-15px -120px;}
.wts{text-align:left;line-height:28px;padding:10px 0 0px 60px;font-size:12px;background:url(/images/ts_bg.png) no-repeat 0 center;color:#060}.wts a{color:#333;}ul#history em{display:block;margin-top:8px;color:#333;font-style:normal;text-indent:5px;}</style></head>\r\n<body leftmargin='0' topmargin='0' bgcolor='#f7f7f7'>".(isset($GLOBALS['ucsynlogin']) ? $GLOBALS['ucsynlogin'] : '')."\r\n<center>\r\n<script>\r\n";
    $htmlfoot  = "</script>\r\n</center>\r\n</body>\r\n</html>\r\n";
    ...
    else
    {
      ...
      function JumpUrl(){
        ...
        $rmsg .= "<div class='ts_b2'><h2>织梦58提示信息！</h2>\");\r\n";
...
{ 
global $dsql; 
$row = $dsql->GetOne("SELECT picname,topid FROM dede_arctype WHERE id= $id"); 
if ($row['topid'] == '0') 
{ 
return $row['picname']; 
} 
...
{ 
$row1 = $dsql->GetOne("SELECT picname FROM dede_arctype WHERE id= $row[topid]"); 
return $row1['picname']; 
} 
}